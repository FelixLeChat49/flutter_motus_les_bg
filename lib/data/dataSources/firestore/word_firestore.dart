...
class WordFirestore {
  ...
  static WordFirestore getInstance() {
    if (_instance == null) {
      _wordRef = _firebaseFirestore.collection('wordsVF').withConverter<Word>(
  ...
  Future<QuerySnapshot<Word>?> getWordOfTheDay() async {
    return await _wordRef.where('activeDate', isEqualTo: DateFormat.yMMMEd().format(DateTime.now())).limit(1).get();
  }
}
